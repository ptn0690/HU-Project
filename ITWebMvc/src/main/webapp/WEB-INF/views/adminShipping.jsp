...
<div>
    		...
    		<form action="DeleteShippingServlet" method="post">
                    <a href="addNewShipping.jsp">Add new </a>
...
<%
    ...
    List<ShippingMethod> shippings = shippingDao.getAllShipping();
    int number = 0;
    for(ShippingMethod each : shippings){
        number++;    
%>                                    
					<tr>
						<td class="auto-style1"><%=number%></td>
                                                <td class="auto-style1"><a href="editShipping.jsp?id=<%=each.getId()%>"><%=each.getMethod()%></a></td>
